...
export default async function ApplicationsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  let query = sb.from("applications")
    .select("id,first_name,surname,email,mobile,status,source,consent_given,consent_at,privacy_notice_version,consent_method,decision_reason,reviewed_at,created_at,participant_id,approved_fee,programmes(name,amount_due)", { count: "exact" });
  query = decided
      ...
      <div className="card card-flush">
        <div className="table-wrap">
          <table className="data">
            ...
            <tbody>
              {(rows ?? []).map(row => {
                const programme = row.programmes as unknown as { name: string; amount_due: number } | null;
                return (
                  <tr key={row.id}>
                    ...
                    <td>{programme?.name}<div className="faint">Programme fee: {formatMoney(programme?.amount_due ?? 0)}</div></td>
                    <td>
                    ...
                    ) : canEditParticipants(user) ? <ApplicationDecision id={row.id} fee={programme?.amount_due ?? 0} consent={row.consent_given && !!row.consent_at} /> : <span className="faint">Finance reviews applications</span>}</td>
                  </tr>
                );
              })}
