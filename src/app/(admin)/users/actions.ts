...
export async function inviteUser(form: FormData) {
 ...
 await sb.from("audit_logs").insert({ actor_id: actor.id, action: "user.invited", entity_type: "user", entity_id: data.user.id, summary: "Staff member or runner invited" });
...
export async function editUser(form: FormData) {
 ...
 const { error } = await sb.from("app_users").update({ role, is_active: active, can_verify: role === "course_admin" && form.get("verify") === "on", programme_ids: programmes }).eq("id", id);
