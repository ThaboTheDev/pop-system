...
- **Public `/register` and runner `/runner/register`** write `applications`, not
  ...
  stored on that application. Duplicate submissions do not create a second row.
...
`npm test` runs the existing operations suite against all eight migrations and a
